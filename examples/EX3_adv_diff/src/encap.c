...
/* LibPFASST sends the data for a encap_data_t through MPI as an array of double.
 * This imposes the following requirements on the data format
 *     1. data for a encap_data_t must be stored in contiguous memory
 *     2. size of one encap_data_t must be a multiple of sizeof(double)
...
size_t ex3_data_get_size(size_t nx) {
    size_t size = sizeof(encap_data_t) + nx * sizeof(double);
...
void cpf_encap_eprint_cb(encap_data_t* data) {
    // prints 5 first and 5 last array members
